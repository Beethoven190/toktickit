...
**Lab 1: TokTickIT (ตอกติ๊กกิต) Full-Stack Hello World Starter**  
**Score:** _____ / 40  
...
| :--- | :--- |
| **Requester, IT Staff, and Administrator can all:** | • View permitted ticket information<br>• Add public comments<br>• Add attachments |
| **Requester can:** | • Create a ticket<br>• View their own tickets<br>• Set Requested Priority<br>• Respond to IT Staff<br>• Confirm or reject a resolution<br>• Request reopening |
| **IT Staff and Administrator can both:** | • Become or change the Ticket Owner<br>• Set IT Priority<br>• Change ticket status<br>• Add internal notes<br>• Add and update actions taken<br>• Resolve and close tickets |
| **Administrator can:** | • View and search users<br>• Create user accounts<br>• Edit user details<br>• Assign a user role<br>• Activate or deactivate accounts<br>• Set a new initial password<br>• Manage Categories and Related Systems |
...
- **Four supported categories:** Loaded from PostgreSQL through the backend via API call.
- **Loading state:** (Hourglass ... "loading") while data is being requested.
- **Error state:** A useful error message when the backend or database is unavailable.
...
> **Note:** Do not substitute another framework, database, ORM, or UI library. Playwright, authentication, ticket creation, and image upload will be introduced in later labs.
...
4. **Create the GitHub Project:** Create a GitHub Project named `TokTickIT Individual Sprints`. Use a Board view with these statuses in this exact order:  
   ...
   - A status moves from `PR Review` to `Fixing` if it did not pass review; otherwise, it moves to `Done`.
   - Issues move from `Started` to `PR Review` and also from `Fixing` to `PR Review`.

```mermaid
flowchart LR
    Backlog["Backlog<br/>(Raw/unclear issues)"] --> Specified["Specified<br/>(Detailed specs clear)"]
    Specified --> Started["Started<br/>(Development underway)"]
    Started --> PR_Review["PR Review<br/>(Peer code review & verification)"]
    PR_Review -- "Fails Review" --> Fixing["Fixing<br/>(Resolving pull request comments)"]
    Fixing -- "Fix Submitted" --> PR_Review
    PR_Review -- "Passes Review<br/>(Issue Merged)" --> Done["Done<br/>(Merged to Next Branch)"]
```
...
- **Acceptance criteria:**
  ...
  - Initial `README.md` setup instructions are present.
...
### Feature Branches & Pull Request Targets
...
## 15. Grading Rubric & Required Submission Evidence

| Part | Points | Required Submission Evidence |
| :--- | :---: | :--- |
| **1. Git Use with Engineering Workflow** | **15** | • **URL List** for your GitHub repository, GitHub Project, all GitHub Issues in your Project, all Pull Requests (feature branches, staging branch, main branch).<br>• **Evidence of GitHub Project usage**, showing list of issues as Kanban board, and final board all in "Done" Kanban state.<br>• **Screenshot evidence of git workflow** by showing your commit history in the final `main` branch showing you created various feature branches that were eventually merged into the staging branch and then the `main` branch.<br>• **Screenshot of entire Directory Structure** of your repository in your IDE aligning with "Required Repository Structure". These files should appear: `docs/lab-01/tests.md`, `docs/lab-01/reviewer.md`, `docs/lab-01/ai_use.md`, `README.md`, and test files under `tests/lab-01/`.<br>• **Rendered `README.md`** (with content as per requirements) and `.gitignore` content.<br>• **PR Review Evidence (5 points):**<br>&nbsp;&nbsp;- Rendered version of `docs/lab-01/reviewer.md` containing reviewer name, student ID, GitHub username, and reviewed PR links.<br>&nbsp;&nbsp;- Evidence that your peer partner approved your submitted PRs: What review comment did your partner give you, and how did you respond?<br>&nbsp;&nbsp;- Evidence that you reviewed and approved your partner's PRs: What review comment did you give your partner, and how did your partner respond?<br>&nbsp;&nbsp;- Screenshots of the board showing all four Issues in the "Done" board in your Kanban. |
| **2. Tests** | **10** | Lab 1 tests are intended to prove that the initial TokTickIT vertical slice works correctly.<br>• **Passing test evidence:** Provide screenshot or copied terminal output showing all tests passed in the `main` branch.<br>• **Rendered `docs/lab-01/tests.md`:** Print rendered version so it's easier to grade. All tests listed should be found in folder `tests/lab-01`.<br><br>*(Example format for `tests.md` below)* |
| **3. AI Use and Reflection** | **5** | • **Rendered `docs/lab-01/ai_use.md`:** Print rendered version that mentions the LLM you used and provides a table of selected key prompts (6-10 is enough). Provide reflection on your experience in improving the prompts.<br><br>*(Example format for `ai_use.md` below)* |
| **4. App Demo** | **10** | • **Screenshot of running browser page with app demo** to show that the two HTTP requests are working. When the app comes on, include a **[Check System]** button with behavior shown below:<br><br>1. **Initial Screen:**<br>```text<br>┌──────────────────────────────────────────────┐<br>│ TokTickIT IT Service Desk                    │<br>│                                              │<br>│ [ Check System ]                             │<br>└──────────────────────────────────────────────┘<br>```<br>2. **Success Case after clicking [Check System]:**<br>```text<br>┌──────────────────────────────────────────────┐<br>│ TokTickIT IT Service Desk                    │<br>│                                              │<br>│ [ Check System ]                             │<br>│                                              │<br>│ System Status: Online                        │<br>│                                              │<br>│ Supported Request Categories                 │<br>│ 1. Account and Access                        │<br>│ 2. Hardware                                  │<br>│ 3. Software                                  │<br>│ 4. Network                                   │<br>└──────────────────────────────────────────────┘<br>```<br>3. **Failure Case after clicking [Check System] (e.g. DB server not started):**<br>```text<br>┌──────────────────────────────────────────────┐<br>│ TokTickIT IT Service Desk                    │<br>│                                              │<br>│ [ Check System ]                             │<br>│                                              │<br>│ System Status: Offline                       │<br>│ Unable to connect to TokTickIT API           │<br>└──────────────────────────────────────────────┘<br>``` |

---

### Appendix A: Example Format for `docs/lab-01/tests.md`
...
### Appendix B: Example Format for `docs/lab-01/ai_use.md`
...
| :--- | :--- |
| **Plan Lab 1 Implementation** | Read the enclosed TokTickIT Lab 1 requirements. Summarize the four GitHub Issues, their dependencies, required outputs, and required automated tests. Propose an implementation order, but do not write code yet.<br><br>**My Reflection:** *This worked in one shot.* |
| **Set Up Full-Stack Project** | Setup the TokTickIT project tech stack as given in Lab 1 using React, TypeScript, Vite, and Bootstrap for the frontend, and Node.js, Express, and TypeScript for the backend. Configure PostgreSQL and Prisma. Use the required folder structure. Do not add functionality beyond the Lab 1 scope.<br><br>**My Reflection:** *I had to follow up this prompt with a few more to detail ...* |
| **Implement Health Check** | Add `GET /api/health` to the existing Express backend. It must ...<br><br>**My Reflection:** *...* |
| **Implement Category Feature** | Create the Prisma Category model, ...<br><br>**My Reflection:** *...* |
| **Build and Test Check System UI** | Create a Bootstrap-based page with `[Check System]` button. When clicked, show a loading state, call ...<br><br>**My Reflection:** *...* |
| **Review Final Lab 1 Work** | Review the completed TokTickIT Lab 1 implementation against all acceptance criteria ...<br><br>**My Reflection:** *...* |
